...
class SQLServer(AgentCheck):
    ...
    def __init__(self, name, init_config, agentConfig, instances=None):
        ...
        self.instances_per_type_metrics = {}
...
class SqlOsVirtualFileStat(SqlServerMetric):
    ...
    def __init__(self, connector, datadog_name, sql_name, base_name,
        ...
        super(SqlServerMetric, self).__init__(connector, datadog_name, sql_name,
                                              base_name, report_function, instance,
                                              tag_by, logger)
    ...
    def fetch_metric(self, cursor, rows, columns, tags):
        ...
        for row in rows:
            dbid = row[dbid_ndx].strip()
            fid = row[fileid_ndx].strip()
            value = row[column_ndx].strip()
            ...
            if self.fid and self.fid != fid:
                continue
            metric_tags = tags
            metric_tags = metric_tags + ['database_id:%s' % (str(dbid))]
            metric_tags = metric_tags + ['file_id:%s' % (str(fid))]
...
class SqlOsMemoryClerksStat(SqlServerMetric):
    ...
    def fetch_metric(self, cursor, rows, columns, tags):
        ...
        for row in rows:
            ...
            node_id = row[memnode_index].strip()
